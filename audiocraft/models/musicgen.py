...
class MusicGen:
    ...
    def generate_music_for_duration(self, description: str, melody: tp.Optional[tp.Tuple[int, np.ndarray]], window_len_secs: float, total_duration_seconds: float, slide_seconds: float, progress: bool = True) -> torch.Tensor:
        """
        Generate music for a longer duration using the MusicGen model.

        Args:
            description (str): The description to condition the model.
            melody (tp.Optional[tp.Tuple[int, np.ndarray]]): A tuple of sample rate and the melody waveform. None if no melody is provided.
